...
fn run_drain_loop(mut session: wf64::Wf64Session) {
    ...
    // Cooperative-agent pump. Convert this worker thread to the operator fiber so
    // it can run agent green threads (lib/agents.f). Until a pane spawns an agent,
    // `ready_count()` is 0 and the loop blocks on next_event exactly as before —
    // so behaviour is unchanged for the existing (no-agent) IDE. Once agents are
    // runnable, we drain events without blocking and advance them with run-slice,
    // so a long-running pane (e.g. a Mandelbrot agent that `pause`s) no longer
    // freezes the console.
    let _ = session.eval("(agent-init) drop\n");

    loop {
        let runnable = wf64::agents::ready_count() > 0;
        ...
        if wf64::agents::ready_count() > 0 {
            // Advance every runnable agent one cooperative slice.
            let _ = session.eval("run-slice\n");
        }
    }
}
