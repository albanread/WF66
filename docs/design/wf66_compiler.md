...
differential oracle.** You can optimize aggressively because the fuzzer proves the
**observable Forth state** — data stack, program-defined memory, output — is
unchanged (§9), even though generated bytes and register use differ. It compares
semantics, *not* scratch registers or instruction count: a too-strict oracle would
pin WF66 to WF65's incidentals and forbid the register allocation that is the
point. Hand-tuned commercial optimizers are least trusted exactly here.
...
## 3. stack-flow → SSA (the per-block symbolic stack, generalized to the CFG)

- **Intra-block:** the symbolic-stack scheduler (the `StackCache` from WF65's
  optimizer lineage) — each push is a fresh SSA value; `swap`/`dup`/`over`/`rot`
  become **renames** (zero cost); `force_settle` is the lowering postcondition.
...
Together these passes **subsume the entire WF65 peephole/replay layer** as
whole-definition rewrites — `try_fold_literal`, the `LAST_*` compare/dup/addr
tails, `OPT_FENCE`, and the `bl`/`true`/`false` ordering tricks all become IR
passes. WF66 runs **no peephole pass alongside** the IR optimizer; the old layer
is deleted, not retained (charter *Replaced*).
...
- **`hotvariable` pinning is subsumed**: a variable whose live range spans a loop
  ...
  (`register_pinning_v1`) folds into general allocation — or remains a hint. Because
  a promoted variable is *named, persistent* memory the oracle compares, the
  allocator writes it back before any THROW-capable op in its range so it is never
  stale on `THROW` — see charter *Exceptions* (anonymous data-stack values carry no
  such obligation).
...
- **Per-primitive emit templates.** Each `Inline(fop)` carries a JASM template
  ...
  remain the semantic source of truth and the template source** (the charter's
  **Two Sources of Truth** contract — kept honest by a golden test that runs the
  template and the `proc` body and compares observable state).
...
- **`force_settle`** at region end = the lowering postcondition (canonical stack
  state) — the same settle applied at class-(c) boundaries (§3).
...
## 9. Verification — WF65 is the oracle (per the charter's *Test Strategy*)

The oracle compares **observable Forth state, not machine incidentals** — full
contract in the charter's *Test Strategy*. In short:

- **Differential oracle:** identical source → identical *observable* state (data
  stack + depth, program-defined memory, output). Scratch registers, spill / sub-
  stack memory, instruction bytes, and instruction *count* are **not** compared —
  pinning them would be a false oracle that forbids register allocation. WF65 is a
  cross-check that defers to Forth-2012, not the spec.
- **Pure optimizer unit tests** — `IR → IR`, no JIT, no session.
- **Differential state fuzzer:** random definitions (incl. control-flow nests);
  compare the observable state above against WF65. Plus the **value-oracle fuzzer**
  (random pure expressions vs an independent Rust evaluator — proven in WF65 to
  catch the consumption-bug class).
- **Static stack-balance / effect check** — falls out of stack-flow; a *new* bug
  class WF65 caught only dynamically.
- **Golden lowering tests** for the fixed templates (template vs `proc` body).
  Bench-corpus instruction/throughput is a **regression tripwire, not a per-word
  gate** — WF66 codegen changes by design, so only a corpus-level wrong-direction
...
The authoritative phase / sprint sequence is **[wf66_roadmap.md](wf66_roadmap.md)**;
Phase 4 (the CFG + register-allocation jump) is detailed in
**[wf66_phase4_plan.md](wf66_phase4_plan.md)**. This document is the *architecture*
those phases build out: Phases 0–3 land §§1–4 (token IR, stack-flow, the optimizer),
Phase 4 lands §5 (cross-block register allocation), Phase 5 the locals/float cases.
`CODE:` opaque nodes and the class-(c) settle-fallback preserve full Forth semantics
at every phase.
...
- **Exceptions vs register residency** → anonymous values are transparent (`THROW`
  restores depth from the catch frame and discards higher cells); a promoted *named*
  cell is written back before any THROW-capable op so program-defined memory stays
  canonical (charter *Exceptions*). The differential fuzzer exercises `CATCH`/`THROW`
  and fault paths to keep this honest.
...
- **The moat** — a complete, frozen WF65 as a differential *observable-state*
  oracle for every optimization. Be more aggressive, because you can prove you
  didn't change the answer (without pinning the codegen that you want to change).
