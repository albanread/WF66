...
WF64 is a 64-bit subroutine-threaded Forth for Windows x86-64, built on the JASM macro assembler and continuous JIT. The binary is a Direct2D MDI GUI with an interactive REPL, a live stack viewer, a diagnostic log pane, and a crash-dump viewer.
...
- **JASM kernel.** The primitives are written in MASM-flavoured assembly using the JASM macro assembler, which expands into machine code at process start. Editing a primitive means editing a `.masm` file and running `cargo run` — no separate assembler invocation, no `link` step.
